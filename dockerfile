ARG BASE_IMAGE=Alfanaannet/UptimeK:base2
...
############################################
FROM Alfanaannet/UptimeK:builder-go AS build_healthcheck

############################################
# Build in Node.js
############################################
FROM Alfanaannet/UptimeK:base2 AS build
USER node
WORKDIR /app
...
LABEL org.opencontainers.image.source="https://github.com/Alfanaannet/UptimeK"
...
############################################
FROM Alfanaannet/UptimeK:base2 AS pr-test2
WORKDIR /app
ENV PUPPETEER_SKIP_CHROMIUM_DOWNLOAD=1
...
RUN git clone https://github.com/Alfanaannet/UptimeK.git .
RUN npm ci
